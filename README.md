...
- **`/html-report`** generates a self-contained HTML dashboard from `job_search_tracker.csv`, application archives, and optional `application_events.csv` history — stat cards, funnel charts, experiment comparisons, time-in-stage metrics, and a filterable applications table. Opens directly in a browser, fully offline. Re-run it any time after `/apply` or `/outcome` adds new entries.
...
│   ├── application_history.py          # Append-only event and experiment recorder
...
├── application_events.csv             # Private append-only event history (gitignored)
├── .job-search-profile.json           # Private profile/experiment identity (gitignored)
...
6. **Compile and inspect** both PDFs: lualatex for the CV, xelatex for the cover letter. Claude reads the rendered pages and iterates until the CV is preferably 1 page and never more than 2, retaining a justified second page when cutting it would remove relevant evidence. The cover letter remains exactly 1 page with the signature visible and fonts consistent.
