...
describe("stepstone CLI flag validation", () => {
  ...
  test("search without --query exits 1 with NO_QUERY", async () => {
    const result = await runCLI(["search", "-l", "Köln"]);
    expect(result.exitCode).toBe(1);
    expect(parsedStderr(result.stderr).code).toBe("NO_QUERY");
  });

  test("search without --location exits 1 with NO_LOCATION", async () => {
    const result = await runCLI(["search", "-q", "test"]);
    expect(result.exitCode).toBe(1);
    expect(parsedStderr(result.stderr).code).toBe("NO_LOCATION");
  });
  ...
  test("non-numeric --page exits 1 with BAD_ARG", async () => {
    const result = await runCLI(["search", "-q", "test", "-l", "Köln", "--page", "abc"]);
    expect(result.exitCode).toBe(1);
  ...
  test("non-numeric --limit exits 1 with BAD_ARG", async () => {
    const result = await runCLI(["search", "-q", "test", "-l", "Köln", "--limit", "xyz"]);
    expect(result.exitCode).toBe(1);
    const err = parsedStderr(result.stderr);
    expect(err.code).toBe("BAD_ARG");
