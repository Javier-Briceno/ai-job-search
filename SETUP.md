...
| `.job-search-profile.json` | Private stable profile ID and active experiment (gitignored) |

`/setup` initializes the tracking identity with experiment `baseline`. Application
events then accumulate in gitignored `application_events.csv`, including the
workflow commit that produced each draft or recorded stage. Before testing a
workflow optimization, select a new label with:

```bash
python tools/application_history.py experiment --id <experiment-id>
```

Use a separate clone or Git worktree for each candidate. Branches isolate
committed profile files, but ignored trackers, documents, and event history stay
in the current working directory and otherwise mix across branch switches.
